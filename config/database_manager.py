        ...
        access_events = pd.DataFrame(
            {
                ...
                "access_result": [
                    "Granted" if i % 4 != 0 else "Denied" for i in range(100)
                ],
                "badge_status": [
                    "Valid" if i % 5 != 0 else "Invalid" for i in range(100)
                ],
            }
        )
...
class DatabaseManager:
    ...
    def from_environment() -> DatabaseConfig:
        ...
        db_type = manager.get("DB_TYPE", "mock").lower()

        if db_type == "postgresql":
            return DatabaseConfig(
                db_type="postgresql",
                host=manager.get("DB_HOST", "localhost"),
                port=int(manager.get("DB_PORT", "5432")),
                database=manager.get("DB_NAME", "yosai_intel"),
                username=manager.get("DB_USER", "postgres"),
                password=manager.get("DB_PASSWORD", ""),
                pool_size=int(manager.get("DB_POOL_SIZE", "5")),
            )
        elif db_type == "sqlite":
            return DatabaseConfig(
                db_type="sqlite", file_path=manager.get("DB_FILE", "yosai_intel.db")
            )
        else:
