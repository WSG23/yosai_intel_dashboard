...
class EntryClassificationService:
    ...
    SECURITY_KEYWORDS = {
        'high': ['server', 'data', 'telecom', 'electrical', 'mechanical', 'executive', 'ceo', 'finance', 'hr', 'secure', 'restricted'],
        'medium': ['office', 'meeting', 'conference', 'storage', 'supply', 'break', 'kitchen'],
        'low': ['lobby', 'entrance', 'public', 'visitor', 'reception', 'restroom', 'hallway', 'corridor']
    }

    FLOOR_PATTERNS = [
        r'\b(\d+)(?:st|nd|rd|th)?\s*fl(?:oor)?\b',
        r'\bf(\d+)\b', r'\b(\d+)f\b',
        r'\blevel[\s\-_]*(\d+)\b',
        r'\b(\d+)-\d+\b', r'\b(\d+)\.\d+\b'
    ]
    ...
    def _detect_floor(self, device_name: str) -> Optional[int]:
        """Detect floor number from device name"""
        for pattern in self.FLOOR_PATTERNS:
            matches = re.finditer(pattern, device_name)
    ...
    def _detect_entry(self, device_name: str) -> bool:
        ...
        entry_terms = ['entry', 'entrance', 'enter', 'in', 'lobby', 'reception', 'front', 'main']
    ...
    def _detect_stairwell(self, device_name: str) -> bool:
        ...
        return any(term in device_name for term in ['stair', 'stairs', 'stairwell', 'steps'])

    def _detect_fire_escape(self, device_name: str) -> bool:
        """Detect fire escape"""
        return any(term in device_name for term in ['fire', 'emergency', 'evacuation', 'escape'])
    ...
    def _calculate_confidence(self, device_name: str) -> float:
        ...
        floor_detected = any(re.search(pattern, device_name) for pattern in self.FLOOR_PATTERNS)
