...
def create_dual_file_uploader(upload_id="upload-data"):
    """Create existing dual file uploader - shows CSV/Database options FIRST"""
    try:
        return html.Div([
            # Upload heading
            html.Div([
                html.H2("📁 Choose Your Data Source", className="upload-heading"),
                html.P(
                    "Select your preferred upload method to begin analysis",
                    className="upload-subheading",
                ),
            ], className="upload-header"),

            # EXISTING dual upload interface
            html.Div([
                # File Upload Box (CSV/JSON/Excel) - PRIMARY
                dcc.Upload(
                    id=upload_id,
                    children=html.Div([
                        html.I(className="fas fa-cloud-upload-alt upload-icon"),
                        html.Div("File Upload", className="upload-box-title"),
                        html.Div("Drop files here or click to browse", className="upload-box-subtitle"),
                        html.Div([
